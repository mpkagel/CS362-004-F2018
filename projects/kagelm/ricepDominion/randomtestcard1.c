...
int main() {
	...
	for (n = 0; n < numTests; n++) {
		...
		for (j = 0; j < g.numPlayers; j++) {
			g.deckCount[j] = floor(Random() * 80);
			g.discardCount[j] = floor(Random() * 300);
		...
		outcome = cardEffect(smithy, 0, 0, 0, &g, handPos, 0);
		
		// Artifically make the test game state have the cards that were drawn
		// from the smithy card in the game state g	
		for (j = 0; j < card_count; j++) {
			...
			testg.handCount[p]++;
			testg.hand[p][testg.handCount[p] - 1] = testg.deck[p][testg.deckCount[p] - 1];
			testg.deckCount[p]--;	
		}
		testg.playedCardCount++;
		testg.playedCards[testg.playedCardCount - 1] = testg.hand[p][handPos];
		testg.hand[p][handPos] = testg.hand[p][testg.handCount[p] - 1];
		testg.handCount[p]--;	

		result = compareGameStateTest1(numPlayers, g, testg, p, handPos, n, test_failures);
		if (!result) {
			printf("****** Test %d Player %d plays Smithy from handPos %d ******\n", n, p, handPos);
			printf("TEST failed\n");
			all_test_result = 0;
			printf("------- gameState g... -------\n");
			printRandomState(g);
			...
			sprintf(message, "FAIL: Test %d Smithy card outcome should be 0\n", n);
	...
	if (all_test_result) {
		printf("\n+++++ : PASS, randomtestcard1 passed.\n");
	} else {
		printf("\n+++++ : FAIL, randomtestcard1 failed.\n");	
	}
