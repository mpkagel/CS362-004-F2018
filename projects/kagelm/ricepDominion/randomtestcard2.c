...
int main() {
	...
	for (n = 0; n < numTests; n++) {
		...
		outcome = cardEffect(village, 0, 0, 0, &g, handPos, 0);
		
		// Artifically make the test game state have the card that was drawn
		// from the village card in the game state g	
		testg.numActions += 2;
		...
		if (!result) {
			printf("****** Test %d Player %d plays Village from handPos %d ******\n", n, p, handPos);
			printf("TEST failed\n");
			all_test_result = 0;
			printf("------- gameState g... -------\n");
			printRandomState(g);
			...
			sprintf(message, "FAIL: Test %d Village card outcome should be 0\n", n);
	...
	if (all_test_result) {
		printf("\n+++++ : PASS, randomtestcard2 passed.\n");
	} else {
		printf("\n+++++ : FAIL, randomtestcard2 failed.\n");	
	}
